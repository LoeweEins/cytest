...
force_tags = ["Pricing", "价格"]
default_tags = ["回归"]

def suite_setup():
    GSTORE.pricing_page_size = int(os.getenv("PRICING_PAGE_SIZE", "3"))
    ...
    if GSTORE.saleor_api_callable:
        try:
            ...
            INFO(f"[31_pricing] GraphQL endpoint 不可 POST：{e}")
    ...
    if not GSTORE.saleor_access_token:
        ...
        if email and pwd and GSTORE.saleor_api_callable:
            try:
                ...
                INFO(f"[31_pricing] tokenCreate 失败：{e}")
            else:
                token = node.get("token") if isinstance(node, dict) else None
                GSTORE.saleor_access_token = token

    INFO(
        f"[31_pricing] suite_setup, page_size={GSTORE.pricing_page_size}, "
...
def suite_teardown():
    INFO("[31_pricing] suite_teardown")
