...
force_tags = ["Inventory", "库存"]
default_tags = ["回归"]

def suite_setup():
    GSTORE.inventory_page_size = int(os.getenv("INVENTORY_PAGE_SIZE", "5"))
    ...
    if GSTORE.saleor_api_callable:
        try:
            ...
            INFO(f"[32_inventory] GraphQL endpoint 不可 POST：{e}")
    ...
    if not GSTORE.saleor_access_token:
        ...
        if email and pwd and GSTORE.saleor_api_callable:
            try:
                ...
                INFO(f"[32_inventory] tokenCreate 失败：{e}")
            else:
                token = node.get("token") if isinstance(node, dict) else None
                GSTORE.saleor_access_token = token

    INFO(
        f"[32_inventory] suite_setup, page_size={GSTORE.inventory_page_size}, "
...
def suite_teardown():
    INFO("[32_inventory] suite_teardown")
