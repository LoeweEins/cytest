...
def suite_setup():
    ...
    if GSTORE.saleor_api_callable:
        try:
            ...
            INFO(f"[90_admin] GraphQL endpoint 不可 POST：{e}")
            GSTORE.saleor_api_callable = False

    # admin 套件强依赖 staff token
    ...
    INFO(
        f"[90_admin] suite_setup, url={GSTORE.saleor_graphql_url or '<EMPTY>'}, callable={GSTORE.saleor_api_callable}, "
