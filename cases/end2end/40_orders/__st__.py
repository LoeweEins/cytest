...
def suite_setup():
    ...
    if GSTORE.saleor_api_callable:
        try:
            ...
            INFO(f"[40_orders] GraphQL endpoint 不可 POST：{e}")
            GSTORE.saleor_api_callable = False

    # orders 查询通常需要 staff 权限；预先拿 token
    ...
    if not GSTORE.saleor_access_token:
        ...
        if email and pwd and GSTORE.saleor_api_callable:
            try:
                ...
                INFO(f"[40_orders] tokenCreate 失败：{e}")
            else:
                token = node.get("token") if isinstance(node, dict) else None
                GSTORE.saleor_access_token = token

    INFO(
        f"[40_orders] suite_setup, url={GSTORE.saleor_graphql_url or '<EMPTY>'}, "
        f"callable={GSTORE.saleor_api_callable}, token={'Y' if GSTORE.saleor_access_token else 'N'}"
    )
